...
jQuery('.reject_payments').click(function (event) {
		...
		$.ajax({
			type : "POST",
			url  : home+"atms/staff/certifier_record/1/2",
			dataType : "JSON",
...
jQuery(".poi").click(function(){

   let staffID = this.getAttribute("data-staffid");
   let new_location = $('#parish_' + staffID).val();

if(new_location == '66'){
  $('#parish_' + staffID).css({"color": "red", "border": "2px solid red"});
  return false;
}

   $.ajax({
    type : "POST",
    url  : home+"atms/staff/no_parish_update/",
    dataType : "JSON",
    data : {staff_id:staffID,location:new_location},
    success: function(data){
      //$('#'+data.rejected_record).remove();
      
      $('#'+staffID).fadeOut(300, function(){ $(this).remove();});
    }
  }); 
}); 
